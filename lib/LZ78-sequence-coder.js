...
SequenceCoder.prototype.learn = function (inputSequence, lastIgnition, prevState, pos) {
    ...
    logger.debug(chalk.reset(`input:`), `s=${prevState}, x=\"${inputSequence[pos]}\" at pos=${pos}, \"${inputSequence.map((x,i)=> i==pos ? '('+ x + ')' :x).join("")}\"`)
    ...
    if(nextState.length === 0) {
        ...
        logger.info("Learned phrase:", `${inputSequence.slice(lastIgnition, pos + 1)}`);
